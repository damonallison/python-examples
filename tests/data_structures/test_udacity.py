...
def test_udacity_intro_to_python_data_structures() -> None:
    ...
    # director
    nom_count_dict = defaultdict(int)
    ...
    win_count_dict = defaultdict(int)
    for winner_lst in winners.values():
        for winner in winner_lst:
            win_count_dict[winner] += 1

    # 2: Provide a list with the name(s) of the director(s) with the most
    # Oscar wins. The list can hold the names of multiple directors, since there
    # can be more than 1 director tied with the most Oscar wins.

    max_wins = max(win_count_dict.values())

    most_win_directors = [
        winner for winner, count in win_count_dict.items() if count == max_wins
    ]

    assert ["John Ford"] == most_win_directors
    print(f"{most_win_directors} won the most with {max_wins} wins")
