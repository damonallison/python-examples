...
def test_numpy_datatypes() -> None:
    ...
    # When comparing numpy dtypes, always use `np.issubdtype`
    assert np.issubdtype(no.dtype, np.floating)
    assert np.issubdtype(no.dtype, np.float16)
...
def test_numpy_indexing() -> None:
    ...
    operations like indexing based on integer arrays) creates a copy.
...
def test_numpy_rng() -> None:
    ...
    Python will take ~ 1 second to generate 1_000_000 random integers. numpy
    will take ~20 ms.
    """
...
def test_numpy_ufuncs() -> None:
    """Universal functions (ufuncs) are fast vectoried functions that operate on
    arrays.

    Most ufuncs are unary (which manipulate a value) or binary (two or more
    values).
    """
    ...
    # Some ufuncs are exposed on ndarray. All ufuncs are defined in the np
    # namespace.
    assert a2.sum() == 10
    assert np.sum(a2) == 10
...
def test_descriptive_statistics() -> None:
    ...
    assert np.isclose(np.mean(a), 4.5)
    assert np.isclose(np.median(a), 4.5)
    assert np.isclose(np.std(a), 2.8, atol=0.1)
    ...
    # axis = 0 == compute sum across the rows (collapse columns)
    # axis = 1 == compute sum accross the columns (collapse rows)
    assert np.array_equal(np.sum(a, axis=0), np.array([11, 22]))
    assert np.array_equal(np.sum(a, axis=1), np.array([3, 30]))
